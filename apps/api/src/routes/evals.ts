...
import { twinRepository, feedbackRepository, patternRepository } from '@skytwin/db';
  ...
  router.get('/:userId/confidence', async (req, res, next) => {
    try {
      ...
      for (const inf of profile.inferences) {
        ...
        stats.total++;
        const level = inf.confidence.toLowerCase();
      ...
      const totalWeighted = profile.inferences.reduce((sum, inf) => {
        ...
        return sum + (weights[inf.confidence.toLowerCase()] ?? 0);
      }, 0);
