...
const SAMPLE_ROW = {
  ...
  target_path: "/tmp/skytwin-models/qwen2.5-1.5b-instruct-q4-k-m.gguf",
...
describe("POST /api/embedded-llm/downloads/start", () => {
  ...
  it("returns 404 when model id is not in the registry", async () => {
    mockStartDownload.mockRejectedValue(
      new Error("unknown model id: not-real"),
    );
    const { status, body } = await req(
      buildApp(),
      "POST",
  ...
  it("returns typed required and available bytes when disk preflight fails", async () => {
    mockStartDownload.mockRejectedValue({
      code: "insufficient_disk",
      details: { requiredBytes: 2_000, availableBytes: 1_000 },
    });
    const { status, body } = await req(
      buildApp(),
      "POST",
...
describe("cross-user access is forbidden", () => {
  ...
  it("POST /downloads/start rejects when body userId differs from authenticated user", async () => {
    const { status } = await req(
      ...
      "POST",
      "/api/embedded-llm/downloads/start",
      { userId: OTHER_USER_ID, modelId: "qwen2.5-1.5b-instruct-q4-k-m" },
    );
