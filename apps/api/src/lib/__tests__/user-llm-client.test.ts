...
describe('per-user LLM composition root', () => {
  ...
  it('installs the receipt trace sink at the atomic composition root', async () => {
    const sink = vi.fn();
    const factory = vi.spyOn(LlmClient, 'forReasoningMode');
    await resolveUserLlmClient('user-1', { onInferenceTrace: sink });
    expect(factory).toHaveBeenCalledWith(
      'on_device',
      [expect.objectContaining({ name: 'embedded' })],
      'user-1',
      { onInferenceTrace: sink },
    );
    factory.mockRestore();
  });
  ...
  it('does not construct a client for an unknown persisted mode', async () => {
    snapshotMock.mockResolvedValue({
      providers: [localRow],
      reasoningMode: { mode: 'legacy_local', requires_confirmation: false },
    });
    await expect(resolveUserLlmClient('user-1')).resolves.toMatchObject({
      state: 'policy_blocked', client: null,
      reason: expect.stringMatching(/canonical reasoning mode/i),
    });
  });
