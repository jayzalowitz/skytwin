...
- Inbox-Intelligence briefing — a daily/weekly digest that splits **to-dos (act)** from **topics (FYI)**, cites the source signal behind every item, surfaces memory-derived action opportunities with IronClaw/OpenClaw routing, and offers a "Power view" toggle for the technical detail behind each call
