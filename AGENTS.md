...
| `@skytwin/llm-client` | Unified, mode-scoped LLM client (Anthropic, OpenAI, Google, Ollama, embedded). Enforces explicit execution-location boundaries, blocks unpriced unattended inference, retains sanitized fallback/privacy metadata, and provides per-provider circuit breakers, SSRF-safe URL validation, prompt building, response parsing, and cost helpers. No SDK dependencies. |
...
| `@skytwin/connectors` | Gmail, Google Calendar, and mock signal connectors with account-bound OAuth token management (DbTokenStore). Verified connector observations carry an owner-scoped opaque resource reference; raw provider IDs stay at the repository boundary. Stamps every signal with an `AuthoringTier` (six tiers from `user_sent_originated` to `inbox_automated`) so retrieval can weight self-authored content above broadcast noise (#251 Layer 1). Source: [`packages/connectors/src/gmail-connector.ts`](packages/connectors/src/gmail-connector.ts). |
