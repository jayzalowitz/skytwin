...
| `@skytwin/llm-client` | Unified, mode-scoped LLM client (Anthropic, OpenAI, Google, Ollama, embedded, and TrustedRouter; NEAR AI is staged but runtime-blocked). Enforces explicit execution-location boundaries, including request-scoped local-only Ollama resolution. Adaptive Ollama calls additionally require response-bound server/model identity and therefore fail closed on released stock Ollama builds, which do not expose it. The client blocks unpriced unattended inference and admits confidential providers only through pinned verifier-owned adapters. It retains sanitized fallback/privacy metadata and provides per-provider circuit breakers, SSRF-safe URL validation, prompt building, response parsing, cost helpers, and default email-address masking via `redactPromptPii()` for raw-event and episodic-memory decision-prompt fragments. Other prompt fragments remain outside that masking boundary. |
| `@skytwin/near-confidential` | Fail-closed contract for strict confidential inference. NEAR AI remains runtime-disabled because the public base-CVM evidence does not pin the dynamically selected model/proxy workload, and no packaged verifier transport is admitted. |
| `@skytwin/embedded-llm` | Local-first LLM runtime: non-interactive `llama-completion` text generation, `whisper.cpp` STT (`/api/voice/transcribe`), and Piper TTS (`/api/voice/synthesize`). Managed text models require exact registry size, digest, manifest, and llama.cpp compatibility checks. Workflow authoring additionally requires a model/artifact/runtime build that passed the checked-in semantic gate; ordinary local inference may use an unqualified artifact. The model download and runtime remain separate prerequisites. |
| `@skytwin/explanations` | Generates human-readable explanations for decisions and actions. |
| `@skytwin/connectors` | Gmail, Google Calendar, Outlook mail and calendar (Microsoft Graph delta), and mock signal connectors with provider-aware OAuth token management (DbTokenStore). Stamps every signal with an `AuthoringTier` (six tiers from `user_sent_originated` to `inbox_automated`) so retrieval can weight self-authored content above broadcast noise (#251 Layer 1). |
...
| `@skytwin/memory-gbrain` | Default, gbrain-compatible memory backend (#197). `EmbeddedGbrainMemoryPort` runs in-process against the brain_* CRDB tables. Upstream gbrain v0.50.5.0 supports PGLite/PostgreSQL, not CockroachDB; its unchanged schema/runtime fails the supported CRDB path on PostgreSQL-specific DDL/functions, and its CLI lacks SkyTwin's complete write, episode, and graph contract. The CLI-shellout `GbrainMemoryPort` is a programmatic interoperability adapter, not a backend the SkyTwin runtime factory can select. |
...
| `@skytwin/routines` | Typed read-only workflow providers. `signal_digest.v1` validates and canonicalizes payloads, compiles immutable Watch projections, replays real signals, and computes authoritative semantic diffs. |
...
| App | Purpose |
|-----|---------|
| `api` | HTTP API server exposing decisions, users, webhooks, and adaptive-workflow authoring/replay/version/activation/rollback routes. Includes liveness/readiness health checks and mDNS service advertisement. The public pre-auth `/api/system/recommend-local-model` route evaluates hardware internally but returns only a catalog recommendation and fit reason; it does not expose the raw host profile or prove runtime or workflow-authoring readiness. Model downloads expose no host filesystem paths. |
| `web` | Web dashboard for reviewing decisions, managing preferences and policies, and teaching/replaying/activating immutable signal-digest workflow versions. |
| `worker` | Background job processor for async decisions, feedback, and exact-version-pinned Watch runs. Adaptive runs retain deterministic matching during summary-model outages. Includes startup hang detection and graceful shutdown. |
