...
- Status: **Accepted design — key-broker and ownership prerequisites are partial; source-field encryption and public claims remain blocked**
- Date: 2026-09-10
...
| [`PassphraseVault`](../../apps/desktop/src/passphrase-vault.ts)                         | The desktop can persist a `safeStorage` ciphertext of the passphrase. The renderer can request the plaintext passphrase, then sends it to the API over loopback HTTP. This is not the target broker design below.                                                         |
...
| [`ServiceManager`](../../apps/desktop/src/service-manager.ts) and [`SourceKeyBroker`](../../apps/desktop/src/key-broker.ts) | Electron main now composes a user source-key broker and capability-bound API/worker IPC. No production source-field repository routes through it yet, and the installation root-key hierarchy is not implemented. |
...
`076-installation-ownership-and-content-free-dlq.sql`; validation fails when a
table or column is missing or duplicated. This encryption-only branch
intentionally excludes the separate `072-reasoning-mode-settings.sql`,
`074-inference-receipts.sql`, and `075-inference-receipt-completion.sql`
migrations. The inventory must be regenerated and reviewed atomically when
those branches first share a cumulative schema base. It reconstructs the same
schema-plus-sorted-SQL sequence used by the production
...
- `skytwin-passphrase-vault` currently stores `safeStorage` ciphertext; the
  target replaces remembered-passphrase storage with a device-wrapped random
  user key.
...
repository-relative list from a supplemental scan of recognized SQL-shaped
string/template literals and representative query aliases, plus validated
annotations for the known dynamic seed SQL helpers. The seed helper's annotation
is bound to its runtime table allowlist. The list includes recognized direct
API/worker queries, backup code, and seeds; it is not a complete call graph or a
semantic proof that every dynamically assembled query was found.
...
- `service_credentials`, dynamically populated `credential_requirements`, and
  ...
  Migration 076 gives each table a non-null stable `installation_id`, and every
  repository operation resolves it through the singleton owner; its
  private text/config fields, including the tool credential-requirement list,
  use distinct IRK purposes. They are excluded from
  ...
  The database-side compare-and-swap reset remains possible while locked and
  cascades deletion before minting a new owner; composing that reset with the
  future device-wrapped IRK lifecycle remains pending. Installed service
  names, row counts, timestamps, and safety flags that remain metadata disclose
  which capabilities may be present.
- `worker_dead_letter` remains system-global and is content-free after migration
  076. The global row contains stable job/error codes, attempts, timestamps,
  and opaque record IDs only. Cadence replay re-reads current source-of-truth
  state, so no user payload is required and none is relocated. Legacy raw errors
  and contexts are permanently discarded. Encrypting a global payload
...
Before slice 1 can encrypt a source field, its ownership gate must pass. Migration
076 adds and backfills `installation_id` on the three installation-owned tables,
repository tests bind every operation to the singleton owner, the transactional
reset deletes owned rows while locked, and the global DLQ is content-free. No
failure payload is retained because replay does not consume one. Cross-platform
upgrade/restore evidence and the future IRK/device-reset composition remain
required before installation-owned encryption can be enabled.
...
realpath-contained regular-file references with symlinks rejected, consistency
against its supplemental SQL scan and validated dynamic-SQL annotations,
...
Critical credential/DLQ invariants, explicit installation-owner identifiers,
and metadata dependency resolutions are also hardcoded and mutation-tested.
...
strips comments to avoid prose-only matches, recognizes common direct/aliased
query calls, and fails on several unsupported computed/concatenated shapes it can
see. Scope-sensitive aliases, arbitrary wrappers, generated/external code,
stored procedures, and other JavaScript-equivalent constructions can escape it.
The 94-table/877-column schema-derived ownership inventory is authoritative;
`auditedCallsites` is supplemental review evidence. Security review must combine
it with repository tracing and direct-SQL review; passing automation is not
...
the presence of ciphertext columns, the ownership/DLQ gate, and passing inventory
validation do not prove that production source fields are encrypted.
