...
| [`DesktopKeyBroker`](../../apps/desktop/src/key-broker.ts)                              | The custody kernel implements recovery/device wrappers, purpose-separated keys, context-bound envelopes, child capabilities, and a lock barrier. Its recovery-wrapper store uses the narrow CockroachDB registry subpath without a legacy-store or plaintext fallback. Child bindings start empty; exact live API sessions can receive bounded session authority after independent database revalidation, while worker authority stays empty. Source consumers remain disabled. |
| [`ServiceManager`](../../apps/desktop/src/service-manager.ts)                           | Electron main attaches API and worker children with empty initial owner authority and gives only the API an exact-session CockroachDB verifier. No production source repository consumes the broker; the separate API-local OAuth vault path above is the narrow exception. |
