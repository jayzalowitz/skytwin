...
| [migration 081](../../packages/db/src/migrations/081-worker-dead-letter-content-free.sql) | The system-global worker dead-letter queue retains only constrained job/error codes, bounded attempts, lifecycle timestamps/status, and opaque UUID identifiers. Existing diagnostic strings and JSON are discarded and the source-capable columns are dropped. This closes that ownership prerequisite but does not activate source-field encryption elsewhere. |
...
| [`ServiceManager`](../../apps/desktop/src/service-manager.ts)                           | Electron main attaches the API and worker children to the new source-key broker, but deliberately gives both empty owner grants. No production source operation can request a key through that broker foundation; the separate API-local OAuth vault path above is the narrow exception. |
...
- `worker_dead_letter` remains system-global and is content-free as of migration
  ...
  timestamps, and opaque record IDs only. Legacy diagnostic content is erased;
  the worker does not persist a replacement payload. Any future replay design
  that requires user content must use a user-owned record with `user_id`,
