...
| [`DesktopKeyBroker`](../../apps/desktop/src/key-broker.ts)                              | The custody kernel implements recovery/device wrappers, purpose-separated keys, context-bound envelopes, child capabilities, and a lock barrier. Its recovery-wrapper store now uses the narrow CockroachDB registry subpath without a legacy-store or plaintext fallback; production owner grants and source consumers remain disabled. |
