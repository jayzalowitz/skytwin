...
The release workflow at `.github/workflows/release.yml` already injects certs via env vars (CSC_LINK + CSC_KEY_PASSWORD for both macOS and Windows; APPLE_ID + APPLE_APP_SPECIFIC_PASSWORD + APPLE_TEAM_ID for notarization). Add the secrets to the GitHub repository settings and the next tag-push produces signed artifacts automatically.
...
The `release.yml` workflow takes over: builds macOS / Windows / Linux in parallel; signs with the certs from §1.3; publishes a GitHub Release with the .dmg, .zip, .exe, .AppImage, .deb, .rpm and the electron-updater `latest*.yml` files.
