...
| `@skytwin/llm-client` | Unified, mode-scoped LLM client (Anthropic, OpenAI, Google, Ollama, embedded). Enforces explicit execution-location boundaries, blocks unpriced unattended inference, retains sanitized fallback/privacy metadata, and provides per-provider circuit breakers, SSRF-safe URL validation, prompt building, response parsing, cost helpers, and `redactPromptPii()` — masks email addresses in prompts by default so contacts' addresses don't leak to a cloud LLM (#375). No SDK dependencies. |
