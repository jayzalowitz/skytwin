...
| `@skytwin/llm-client` | Unified LLM client with provider chain (Anthropic, OpenAI, Google, Ollama, embedded). Per-provider circuit breakers, SSRF-safe URL validation, prompt builder, response parser, `estimateLlmCostCents()` helper (embedded/Ollama always return 0), and `redactPromptPii()` — masks email addresses in prompts by default so contacts' addresses don't leak to a cloud LLM (#375). No SDK dependencies. |
