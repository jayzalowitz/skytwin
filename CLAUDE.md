...
| `@skytwin/ironclaw-adapter` | HTTP adapter for the [IronClaw](https://github.com/nearai/ironclaw/) execution server. HMAC-SHA256 auth, retries, circuit breaker. Includes DirectExecutionAdapter fallback, a mock, and owner-bound Gmail Inbox mutation/observation primitives. The gated Gmail archive path currently stops after recording proposal consent: the caller kernel, recovery worker, and feedback projection are intentionally not composed into runtime. |
...
| `@skytwin/llm-client` | Unified, mode-scoped LLM client (Anthropic, OpenAI, Google, Ollama, embedded). Enforces explicit execution-location boundaries (including request-scoped local-only Ollama model resolution), blocks unpriced unattended inference, retains sanitized fallback/privacy metadata, and provides per-provider circuit breakers, SSRF-safe URL validation, prompt building, response parsing, cost helpers, and `redactPromptPii()` — masks email addresses in raw-event and episodic-memory prompt fragments by default (#375); other prompt fragments remain outside that masking boundary. No model-vendor SDK dependencies. |
| `@skytwin/near-confidential` | Fail-closed contract for strict confidential inference. Requires live catalog eligibility, same-connection TLS attestation, pinned measurements/verifier version, and exact-byte response-signature verification. The provider remains disabled until a packaged verifier transport satisfies that contract. |
| `@skytwin/embedded-llm` | Local-first LLM runtime: `llama.cpp` text backend, `whisper.cpp` STT backend (`/api/voice/transcribe`), and Piper TTS backend (`/api/voice/synthesize`). The managed text-model path accepts only the pinned registry artifact after exact size, digest, manifest, and llama.cpp compatibility checks; the model download and llama.cpp runtime are separate prerequisites. Each backend falls back to a `Null*Port` when its binary or model is unavailable. Used by `llm-client` as the `embedded` provider and by `/api/voice/*` for the voice loop. |
| `@skytwin/explanations` | Generates human-readable explanations for decisions and actions. |
| `@skytwin/connectors` | Gmail, Google Calendar, Outlook mail + calendar (Microsoft Graph delta), and mock signal connectors with provider- and account-bound OAuth token management (DbTokenStore, Google + Microsoft). Verified Gmail observations carry an owner-scoped opaque resource reference; raw provider IDs stay at the repository boundary. Stamps every signal with an `AuthoringTier` (six tiers from `user_sent_originated` to `inbox_automated`) so retrieval can weight self-authored content above broadcast noise (#251 Layer 1). Outlook signal sources (`outlook` / `outlook_calendar`) are registered across the downstream source-dispatch sites so Microsoft signals are treated like their Gmail/Google-calendar peers. Source: [`packages/connectors/src/gmail-connector.ts`](packages/connectors/src/gmail-connector.ts). |
...
| `@skytwin/memory-gbrain` | Default, gbrain-compatible memory backend (#197). `EmbeddedGbrainMemoryPort` runs in-process against the brain_* CRDB tables because upstream gbrain's PostgreSQL/pgvector runtime is not CockroachDB-compatible. The CLI-shellout `GbrainMemoryPort` is an explicit opt-in adapter for a separately installed upstream gbrain; SkyTwin never auto-selects it. |
...
| `@skytwin/assistant` | Stateless chat service that wraps `LlmClient` for text-only conversations with optional context enrichment (twin + memory) and action-intent routing (#135). When enriched, replies carry the memories they consulted as `MemorySource[]` (`buildWithSources`) so the chat can cite its evidence. |
...
| `@skytwin/routines` | No-code routines (#519). `parseRoutineSpec()` turns a plain-language ask ("every morning, summarize my calendar conflicts and anything from the finance team") into a structured, schedulable `RoutineSpec` (cadence + signal filter + action). Deterministic (no LLM dependency), so the authoring contract is stable; an LLM-backed parse can layer on later with this as the fallback. Also exports the scheduling + matching engine the worker consumes: `computeNextRun(spec, from, tz?)` (next firing time, in the user's IANA timezone with two-pass DST correction) and `matchesFilter(signal, filter)` (AND across sources/from/keywords/domains, OR within each). v1 is **read-only** — routines `digest`/`notify` on matching signals, never send/reply/schedule/spend (so there is no policy gate). The gated worker scheduler (`SKYTWIN_WATCHES_ENABLED`, default off) now persists an immutable, owner-bound `watch_runs` slot before each exact-window signal read; CockroachDB database-clock leases, fencing tokens, and bounded retries make processing crash-durable without claiming exactly-once external effects. Positive completed runs feed briefing and history surfaces; zero-match audit slots stay internal and expire after 30 days. |
...
| App | Purpose |
|-----|---------|
| `api` | HTTP API server exposing decision endpoints, user management, and webhooks. Includes liveness/readiness health checks and mDNS service advertisement. The public pre-auth `/api/system/recommend-local-model` route evaluates hardware internally but returns only a catalog recommendation and fit reason; it does not expose the raw host profile or prove that the separate llama.cpp runtime is ready. Model downloads expose no host filesystem paths. |
...
| `idle-miner-runner` | Desktop-managed child process that scans approved project roots during OS idle periods and stops on parent control or shutdown. |
