...
| `@skytwin/connectors` | Gmail, Google Calendar, Outlook mail + calendar (Microsoft Graph delta), and mock signal connectors with provider-aware OAuth token management (DbTokenStore, Google + Microsoft). Stamps every signal with an `AuthoringTier` (six tiers from `user_sent_originated` to `inbox_automated`) so retrieval can weight self-authored content above broadcast noise (#251 Layer 1). Outlook signal sources (`outlook` / `outlook_calendar`) are registered across the downstream source-dispatch sites so Microsoft signals are treated like their Gmail/Google-calendar peers. |
