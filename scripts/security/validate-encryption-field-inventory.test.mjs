...
test("SQL callsite discovery recognizes CockroachDB UPSERT statements", () => {
  const candidates = sqlTextCandidates(`
    async function write(client) {
      await client.query(\`UPSERT INTO source_key_deletion_intents (user_id) VALUES ($1)\`);
    }
  `);
  assert.equal(candidates.length, 1);
  assert.match(candidates[0], /UPSERT INTO source_key_deletion_intents/);
});

test("SQL callsite discovery recognizes query aliases and element access", () => {
  for (const source of [
    `const run = client.query; run("SELECT * FROM users");`,
    `import { query as run } from "@skytwin/db"; run("SELECT * FROM users");`,
    `const run = client["query"]; run("SELECT * FROM users");`,
    `const { query: run } = client; run("SELECT * FROM users");`,
    `const first = client.query.bind(client); const run = first; run("SELECT * FROM users");`,
    `let run; ({ query: run } = client); run("SELECT * FROM users");`,
    `const run = client.query["bind"](client); run("SELECT * FROM users");`,
    `const bindKey = "bind"; const run = client.query[bindKey](client); run("SELECT * FROM users");`,
    `const queryKey = "query"; client[queryKey]("SELECT * FROM users");`,
    `client["query"]("SELECT * FROM users");`,
  ]) {
    const candidates = sqlTextCandidates(source);
    assert.equal(candidates.length, 1, source);
    assert.match(candidates[0], /SELECT \* FROM users/);
  }
});

test("concatenated SQL table construction fails closed", () => {
  const schema = new Map([["users", ["id"]]]);
  for (const source of [
    `const table = "users"; client.query("SELECT * FROM " + table);`,
    `const run = client.query; const sql = "DELETE FROM " + table; run(sql);`,
    `const run = client["query"]; let sql; sql = "UPDATE " + table + " SET id = 1"; run(sql);`,
    `const prefix = "SELECT * FROM "; const sql = prefix + table; client.query(sql);`,
    `const fragment = "FROM "; const sql = "SELECT * " + fragment + table; client.query(sql);`,
    `let sql = "SELECT * FROM "; sql += table; client.query(sql);`,
  ]) {
    assert.ok(
      auditDynamicSqlFile("probe.ts", source, schema).errors.some((error) =>
        error.includes("concatenated SQL table construction is unsupported"),
      ),
      source,
    );
  }
...
test("unresolved computed SQL dispatch fails closed", () => {
  const schema = new Map([["users", ["id"]]]);
  const source = `client[queryKey]("SELECT * FROM users");`;
  assert.ok(
    auditDynamicSqlFile("probe.ts", source, schema).errors.some((error) =>
      error.includes("computed SQL dispatch is unsupported"),
    ),
  );
});
...
test("global dead-letter context cannot return to the live schema", () => {
  const inventory = JSON.parse(readFileSync(inventoryPath, "utf8"));
  const schema = extractSchemaColumns();
  schema.get("worker_dead_letter").push("context");

  assert.ok(
    validateInventory(inventory, schema).includes(
      "worker_dead_letter.context: not classified",
    ),
  );
});

test("global dead-letter error messages cannot return to the live schema", () => {
  const inventory = JSON.parse(readFileSync(inventoryPath, "utf8"));
  const schema = extractSchemaColumns();
  schema.get("worker_dead_letter").push("error_message");

  assert.ok(
    validateInventory(inventory, schema).includes(
      "worker_dead_letter.error_message: not classified",
    ),
  );
});

test("installation ownership identifiers must remain explicit metadata", () => {
  const inventory = JSON.parse(readFileSync(inventoryPath, "utf8"));
  ...
  assert.ok(
    validateInventory(inventory, extractSchemaColumns()).includes(
      "service_credentials.installation_id: critical invariant requires locally_exposed_metadata",
    ),
  );
});

test("tool credential requirements cannot be weakened to readable metadata", () => {
  const inventory = JSON.parse(readFileSync(inventoryPath, "utf8"));
  moveFieldToClassification(
    inventory,
    "ironclaw_tools",
  ...
  assert.ok(
    validateInventory(inventory, extractSchemaColumns()).includes(
      "ironclaw_tools.requires_credentials: critical invariant requires encrypted_source",
    ),
  );
});
