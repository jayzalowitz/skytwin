...
const EXPECTED_WARNING =
  "This inventory records current exposure and the accepted target boundary. It is not evidence that source-field encryption is implemented or launch-ready.";
const EXPECTED_SEMANTIC_BASELINE_SHA256 =
  "cf295d9eff4ee313e3274690a82cdd1e25ae7dda0836eac4c25f9c9fcc320115";
 ...
 */
function analyzeSqlTextCandidates(source) {
  const parsed = ts.createSourceFile(
    "sql-callsite.ts",
    source,
    ts.ScriptTarget.Latest,
  ...
  const candidates = [];
  const unsupportedQueryConstructions = [];
  const unsupportedQueryDispatches = [];
  const queryAliases = new Set(["query"]);
  let hasQueryCall = false;
  const sqlStart =
    /^\s*(?:SELECT|INSERT|UPSERT|UPDATE|DELETE|WITH|CREATE|ALTER|DROP|TRUNCATE|FROM|JOIN|INTO|REFERENCES)\b/i;
  function isQueryMember(expression) {
    return (
      (ts.isPropertyAccessExpression(expression) &&
        expression.name.text === "query") ||
      (ts.isElementAccessExpression(expression) &&
        expression.argumentExpression &&
        staticStringValue(expression.argumentExpression) === "query")
    );
  }
  function isQueryReference(expression) {
    ...
    return ts.isCallExpression(expression) &&
      ...
      isQueryReference(expression.expression.expression);
  }
  function collectAliases(node) {
    if (ts.isImportSpecifier(node) &&
      (node.propertyName?.text ?? node.name.text) === "query") {
      queryAliases.add(node.name.text);
    }
    if (ts.isVariableDeclaration(node)) {
      if (ts.isIdentifier(node.name) && node.initializer && isQueryReference(node.initializer)) {
        queryAliases.add(node.name.text);
      }
      if (ts.isObjectBindingPattern(node.name)) {
        for (const element of node.name.elements) {
          ...
          if ((ts.isIdentifier(property) || ts.isStringLiteral(property)) &&
            property.text === "query" && ts.isIdentifier(element.name)) {
    ...
    if (ts.isBinaryExpression(node) && node.operatorToken.kind === ts.SyntaxKind.EqualsToken) {
      ...
      if (ts.isIdentifier(left) && isQueryReference(node.right)) {
        queryAliases.add(left.text);
      }
      if (ts.isObjectLiteralExpression(left)) {
        for (const element of left.properties) {
          if (!ts.isPropertyAssignment(element)) continue;
          const property = memberNameFromName(element.name);
          if (property === "query" && ts.isIdentifier(element.initializer)) {
            queryAliases.add(element.initializer.text);
          }
        }
      }
    }
  ...
  // Iterate to a fixed point so transitive aliases are recognized regardless
  // of declaration order. Query aliases are deliberately provenance-bound to
  // an actual query member (or another proven alias), not accepted by name.
  let priorAliasCount = -1;
  while (priorAliasCount !== queryAliases.size) {
    priorAliasCount = queryAliases.size;
  ...
  function memberNameFromName(name) {
  ...
  function resolveExpression(expression, seen = new Set()) {
    ...
    function find(node) {
      if (ts.isVariableDeclaration(node) && ts.isIdentifier(node.name) &&
        node.name.text === expression.text && node.initializer) {
        candidates.push(node.initializer);
      }
      if (ts.isBinaryExpression(node) &&
        node.operatorToken.kind === ts.SyntaxKind.EqualsToken &&
        ts.isIdentifier(unwrap(node.left)) && unwrap(node.left).text === expression.text) {
    ...
    return candidates.length === 1
      ? resolveExpression(candidates[0], seen)
      : expression;
  }
  function staticStringValue(expression, seen = new Set()) {
    expression = unwrap(expression);
    ...
    seen.add(expression.text);
    return staticStringValue(resolved, seen);
  }
  function staticSqlFragments(expression, seen = new Set()) {
    expression = unwrap(expression);
    if (ts.isStringLiteral(expression) || ts.isNoSubstitutionTemplateLiteral(expression)) {
      return expression.text;
    }
    ...
    if (ts.isBinaryExpression(expression) && expression.operatorToken.kind === ts.SyntaxKind.PlusToken) {
      return `${staticSqlFragments(expression.left, new Set(seen))} ${staticSqlFragments(expression.right, new Set(seen))}`;
    }
    if (ts.isIdentifier(expression) && !seen.has(expression.text)) {
      const resolved = resolveExpression(expression, new Set(seen));
      if (resolved !== expression) {
        seen.add(expression.text);
        return staticSqlFragments(resolved, seen);
      }
    }
    return "";
  }
  function visit(node) {
    if (
      ts.isCallExpression(node) &&
      isQueryReference(node.expression)
    ) {
      hasQueryCall = true;
      const queryInput = node.arguments[0] ? resolveExpression(node.arguments[0]) : undefined;
      if (queryInput && ts.isBinaryExpression(queryInput) &&
        queryInput.operatorToken.kind === ts.SyntaxKind.PlusToken) {
        unsupportedQueryConstructions.push(queryInput);
      }
    } else if (ts.isCallExpression(node) && ts.isElementAccessExpression(node.expression) &&
      node.expression.argumentExpression &&
      staticStringValue(node.expression.argumentExpression) === undefined &&
      node.arguments.some((argument) => sqlStart.test(staticSqlFragments(argument)))) {
      // An unresolved computed dispatch could be a hidden `client[key](sql)`.
      // Treat it as an inventory blind spot instead of guessing that it is safe.
      hasQueryCall = true;
      unsupportedQueryDispatches.push(node.expression);
    }
    // Supplemental defense-in-depth for common concatenation shapes. This is
    // deliberately not described as semantic proof of every JavaScript query.
    if (ts.isBinaryExpression(node) &&
      node.operatorToken.kind === ts.SyntaxKind.PlusToken &&
      /\b(?:FROM|INTO|UPDATE|JOIN|TABLE|REFERENCES|TRUNCATE)\b/i.test(
        staticSqlFragments(node),
      )) {
      unsupportedQueryConstructions.push(node);
    }
    if (ts.isBinaryExpression(node) &&
      node.operatorToken.kind === ts.SyntaxKind.PlusEqualsToken &&
      /\b(?:FROM|INTO|UPDATE|JOIN|TABLE|REFERENCES|TRUNCATE)\b/i.test(
        `${staticSqlFragments(node.left)} ${staticSqlFragments(node.right)}`,
      )) {
      unsupportedQueryConstructions.push(node);
    }
    if (
  ...
  visit(parsed);
  return {
    candidates: hasQueryCall ? candidates : [],
    unsupportedQueryConstructions: hasQueryCall
      ? [...new Set(unsupportedQueryConstructions)]
      : [],
    unsupportedQueryDispatches: hasQueryCall
      ? [...new Set(unsupportedQueryDispatches)]
      : [],
  };
}

export function sqlTextCandidates(source) {
  return analyzeSqlTextCandidates(source).candidates;
}
...
export function auditDynamicSqlFile(repoPath, content, schema) {
  ...
  const sqlAnalysis = analyzeSqlTextCandidates(content);
  const sql = sqlAnalysis.candidates.join("\n");
  ...
  const errors = [];
  for (const _construction of sqlAnalysis.unsupportedQueryConstructions) {
    errors.push(
      `${repoPath}: concatenated SQL table construction is unsupported; use a finite annotated template-literal table set`,
    );
  }
  for (const _dispatch of sqlAnalysis.unsupportedQueryDispatches) {
    errors.push(
      `${repoPath}: computed SQL dispatch is unsupported; use a statically identifiable query method`,
    );
  }
  if (
...
export function validateInventory(inventory, schema) {
  ...
  if (
    inventory.status !==
    "ownership-gate-implemented-encryption-not-implemented"
  ) {
    errors.push(
      "status must describe the implemented ownership gate without claiming source-field encryption",
    );
  }
  ...
  for (const [table, columns] of schema) {
    ...
    if (
      ...
      if (
        ...
        errors.push(
          `${table}: auditedCallsites must match the supplemental static SQL scan`,
        );
      }
  ...
  for (const field of [
    ...
    "ironclaw_tools.requires_credentials",
  ...
  for (const field of [
    "credential_requirements.installation_id",
    "installation_identity.installation_id",
    "ironclaw_tools.installation_id",
    "service_credentials.installation_id",
