/**
 * Grocery Reorder E2E Integration Test
 */
...
describe('Grocery Reorder E2E', () => {
  it('full pipeline: grocery reorder → interpret → decide → explain', async () => {
    ...
    const decision = interpreter.interpret({
      ...
      userId,
      subject: 'Weekly grocery reorder',
      items: ['milk', 'eggs', 'bread'],
      costCents: 2500,
      store: 'LocalMart',
    });

    expect(decision.situationType).toBe(SituationType.GROCERY_REORDER);
    expect(decision.domain).toBe('shopping');
