/**
 * Travel Decision E2E Integration Test
 */
...
describe('Travel Decision E2E', () => {
  it('full pipeline: travel booking → interpret → decide → explain', async () => {
    ...
    const decision = interpreter.interpret({
      ...
      type: 'travel_booking',
      userId,
      subject: 'Flight to NYC - $350 round trip',
      destination: 'New York, NY',
      costCents: 35000,
      travelType: 'domestic_flight',
      dates: { departure: '2026-05-01', return: '2026-05-03' },
    });

    expect(decision.situationType).toBe(SituationType.TRAVEL_DECISION);
    expect(decision.domain).toBe('travel');
