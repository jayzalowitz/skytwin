...
describe('assistantRepository idempotent user append', () => {
  ...
  it('atomically creates a new thread and first keyed message', async () => {
    ...
    mockClientQuery
      ...
      .mockResolvedValueOnce({ rows: [{ ...ROW, request_processing_token: PROCESSING_TOKEN }] });
  ...
  it('reconciles an assistant response by the same user-scoped request key', async () => {
    const assistantRow = { ...ROW, role: 'assistant', content: 'Approval required.' };
    mockClientQuery.mockRejectedValueOnce(new Error('SECRET_MARKER response lost'));
  ...
  it('atomically adopts only a stale unfinished user request', async () => {
    mockQuery.mockResolvedValueOnce({ rows: [{ id: ROW.id }] });
    ...
    expect(mockQuery.mock.calls[0]![0]).toContain("response.role = 'assistant'");
    expect(mockQuery.mock.calls[0]![0]).toContain('request_processing_started_at <');
    expect(mockQuery.mock.calls[0]![0]).toContain("INTERVAL '30 seconds'");
