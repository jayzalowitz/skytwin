...
export interface LifebookMetadata {
  /**
   * #321 user-set importance override. When set + recent (within
   * `decayDays`), wins over the LLM's `importance` pick during
   * extraction-worker upserts. Cleared by calling
   * `clearImportanceOverride` or by setting `decayDays: 0` with no
   * subsequent activity (the worker treats 0 as "never decay until
   * cleared").
   */
  importanceOverride?: {
...
export const lifebookRepository = {
  ...
  async setImportanceOverride(
    ...
    const result = await query<LifebookRow>(
      `UPDATE lifebooks
       SET importance = $3,
           metadata = jsonb_set(metadata, '{importanceOverride}', $4::jsonb, true)
       WHERE user_id = $1 AND domain_name = $2
       RETURNING *`,
      [userId, domainName, value, JSON.stringify(override)],
    );
    return result.rows[0] ?? null;
  },
