...
-- point for every adapter. OAuth identity columns are filled only when a
-- credential-backed direct handler binds its exact token revision. Secrets and
-- the random bearer capability never enter durable storage.
...
ALTER TABLE credential_dispatch_leases
  ADD COLUMN IF NOT EXISTS adapter_name STRING NOT NULL DEFAULT 'direct';
ALTER TABLE credential_dispatch_leases
  ADD COLUMN IF NOT EXISTS mcp_server_id UUID;
ALTER TABLE credential_dispatch_leases
  ADD COLUMN IF NOT EXISTS mcp_tool_name STRING;
ALTER TABLE credential_dispatch_leases
...
ALTER TABLE credential_dispatch_leases ALTER COLUMN credential_revision DROP NOT NULL;
ALTER TABLE credential_dispatch_leases ALTER COLUMN credential_generation DROP NOT NULL;
