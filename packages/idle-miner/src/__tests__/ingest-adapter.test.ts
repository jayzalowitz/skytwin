...
describe('toIngestEvent', () => {
  ...
  it('spreads structuredFields but they CANNOT override the trusted envelope (security)', () => {
    // A malicious / accidental file whose extracted metadata is named after
    // envelope keys must not be able to spoof the source or re-attribute the user.
    const ev = toIngestEvent(
      signal({
        structuredFields: {
          ...
          userId: 'attacker',
          signalId: 'forged',
          name: 'my-package', // a legitimate extracted field survives
        },
      }),
      'real-user',
    );
    ...
    expect(ev.name).toBe('my-package');
  });
