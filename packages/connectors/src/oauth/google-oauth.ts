...
export async function refreshAccessToken(
  ...
  const abortFromCaller = () => controller.abort(transport.signal?.reason);
  transport.signal?.addEventListener('abort', abortFromCaller, { once: true });
  ...
  try {
    ...
    transport.signal?.removeEventListener('abort', abortFromCaller);
  }
