...
import { DEFAULT_OLLAMA_BASE_URL, isLoopbackProviderUrl } from './url-validation.js';
...
function isLoopbackOllama(provider: ProviderEntry): boolean {
  ...
  return isLoopbackProviderUrl(provider.baseUrl ?? DEFAULT_OLLAMA_BASE_URL);
}
...
/**
 * Classify an observed provider execution from the typed privacy facts captured
 * at the provider boundary. Requested mode is a policy input, not evidence of
 * where execution occurred.
 */
export function inferenceStatusForExecution(
  execution: ProviderExecutionMetadata,
): Extract<InferenceReceiptStatus, 'on_device' | 'conventional'> {
  const executionClass = executionClassForMetadata(execution);
  if (executionClass === 'on_device' && execution.reasoningMode === 'on_device') {
    return 'on_device';
  }
  if (executionClass === 'conventional_cloud') return 'conventional';
  throw new ProviderModePolicyError(
    'cross_mode_provider',
    'Provider execution metadata cannot produce a supported inference receipt status',
    execution.provider as AIProviderName,
  );
}

/** Derive observed execution class without consulting provider or endpoint labels. */
export function executionClassForMetadata(
  execution: ProviderExecutionMetadata,
): InferenceExecutionClass {
  ...
  if (
    ...
    execution.verificationStatus === 'not_applicable'
  ) {
    return 'on_device';
  }
  if (
    ...
    capabilities.networkScope === 'external' &&
    (capabilities.confidentiality === 'provider_standard' ||
      capabilities.confidentiality === 'operator_declared') &&
    execution.verificationStatus === 'not_applicable'
  ) {
    return 'conventional_cloud';
  }
  if (
    ...
    return 'verified_confidential';
  }
  throw new ProviderModePolicyError(
    'cross_mode_provider',
    'Provider execution metadata cannot produce a truthful execution class',
    execution.provider as AIProviderName,
  );
}
  ...
  if (mode === 'on_device') {
    ...
    for (const provider of providers) {
      ...
      if (capabilities.executionLocation !== 'remote_service' ||
        ...
        throw new ProviderModePolicyError(
          'cross_mode_provider',
          `Provider ${provider.name} is not eligible for bring-your-own-provider reasoning`,
          provider.name,
        );
      }
    }
