...
Most items closed in PR #126; the rest landed across PRs #127–#131
(v0.5.1.0–v0.5.4.0). One P3 remains open, blocked on a hosted-preview
product decision rather than engineering work.

- [x] **P2**: ~~Split `renderDashboard`~~ **Completed:** v0.5.3.0 (2026-04-29) — Pragmatic split: presentation moved to `dashboard-view.js` (pure render helpers + handlers + `initDashboardGlobals`); `dashboard.js` shrinks from 1023 → 483 lines focused on data flow + lifecycle. Full compute/view/effects extraction left for a future pass when there's a concrete testing reason; today's win is the cognitive-load reduction. Zero runtime behavior change.
- [x] **P2**: ~~Migrate the remaining inline `onclick=...` patterns~~ **Completed:** v0.5.4.0 (2026-04-29) — 40 sites migrated to `data-action="…"` + delegated `addEventListener` bindings across approvals/settings/decisions/setup/dashboard/dashboard-view. Each page wires one document-level click delegator gated on `window.location.hash`. Listener-leak + namespace-collision fixes in the same PR after /review.
...
- [x] **P3**: ~~Guarded packaged sample~~ **Completed (Unreleased):** packaged desktop provisions the fictional profile only on its attested bundled database and mints a four-hour credential fixed to the reserved `is_demo` identity. The credential uses an explicit read allowlist and never enables the development authentication bypass. Approve, reject, correct, reset, and learn interactions run in a separate loopback-only, non-persistent simulation with no connector, provider, credential, database-write, or execution-adapter access. Hosted multi-replica operation remains a separate product decision tracked under #630. **Files:** apps/api/src/routes/demo.ts, apps/api/src/auth/demo-session.ts, apps/api/src/services/sample-simulation.ts, apps/web/public/js/sample-session.js
...
- [x] **P2**: ~~Document `TRUST_PROXY_HOPS` deployment guidance~~ **Completed:** v0.5.2.0 (2026-04-29) — README "Deployment" section with topology table + verification curl, plus expanded inline doc on `apps/api/src/index.ts:79`. Hardened the parse to fall back to 0 with console.warn on invalid input.
- [ ] **P3**: Multi-instance preview rate limiting. `globalPreviewTimestamps` is process-local — N API replicas multiply the global hourly cap, and a restart clears it. This does not weaken the packaged read-only session, but it remains a concern if the unauthenticated `/api/v1/demo/preview` inference route is exposed through a hosted multi-replica deployment. **Blocked on:** the product decision to host that preview publicly. **Lean:** CockroachDB row + atomic increment (project already has CRDB; no new infra dependency). **Files:** apps/api/src/routes/demo.ts
