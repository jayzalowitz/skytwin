...
- [x] **P3**: ~~Guarded packaged read-only sample foundation~~ **Completed (Unreleased):** packaged desktop provisions the fictional profile only on its attested bundled database and mints a four-hour credential fixed to the reserved `is_demo` identity. The credential uses an explicit read allowlist and never enables the development authentication bypass. Hosted multi-replica operation and simulated approve/reject/correct/learn interactions remain separate product decisions tracked under #630. **Files:** apps/api/src/routes/demo.ts, apps/api/src/auth/demo-session.ts, packages/db/src/seeds/packaged-sample.ts
