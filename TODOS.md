...
(v0.5.1.0–v0.5.4.0). Two P3s remain open — both blocked on a product
...
- [ ] **P3**: Real production tour mode — short-lived demo session token, or `/api/v1/demo/dashboard` aggregate read-only endpoint. Currently tour mode auto-disables in production (auth bypass not active), which is honest but cuts off the marquee feature for non-localhost deploys. **Blocked on:** product decision about whether to host a public demo at all (e.g., `demo.skytwin.dev`); the local install one-liner already lands users on a populated tour-mode dashboard, so this only matters if/when there's a hosted demo. **Lean:** short-lived demo session token (option a) — preserves the marquee experience, smaller UI branching cost than the read-only aggregate endpoint. **Files:** apps/api/src/routes/demo.ts, apps/api/src/middleware/session-auth.ts
