...
| 2026-06-14 | Wired the palette into the app: **Quiet Confidence** (the new-user default theme) + the base `:root` + the digest tokens now carry these exact values | The system existed only on the digest's scoped tokens while the rest of the app + the default theme still rendered `#6c7bff`/`#6366f1`. QC's "calm/minimal" aesthetic IS this system, so it becomes the launch face; body font set to Geist (was the blacklisted system-ui). Mission Control / Warm Glass / High Contrast remain opt-in alternative aesthetics. |
