...
- **A canonical packaged-sample evidence verifier is now implemented for the native release matrix.** It discovers the current tag-run artifact through GitHub, validates and derives one executable from the macOS ZIP, Windows NSIS installer, or Linux AppImage, launches it in a fresh isolated profile, and writes the current structured machine-report contract only after populated reads, contained simulation commands, session isolation, authorization denials, reset, disposal, and pre/post identity checks pass. This source does not certify a published artifact: `sample.packaged-account-free` remains limited and release remains blocked until a tagged run produces and the publisher validates all three platform reports. See [`docs/sample-release-evidence.md`](docs/sample-release-evidence.md).
