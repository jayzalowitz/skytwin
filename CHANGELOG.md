...
- **`docs/technical-spec.md` API section synced to what actually shipped.** The endpoint reference was on the pre-consolidation `/api/v1/...` namespace and missing every endpoint that landed during Tier-2 polish. Updated to match `apps/api/src/index.ts` post-Tier-2 — most routes live under `/api/...` (no `v1` prefix); the `/api/v1/...` namespace is reserved today for the Ask, Briefings, and Skill-Gaps routers that pre-dated the API consolidation. Documented every endpoint that landed in this run: `DELETE /api/users/:userId` right-to-erasure (#376), `PUT /api/users/:userId/autonomy-pause` per-user pause (#379), `GET /api/twin/:userId/autonomy-state` + `GET /api/twin/:userId/progress` for the pause banner and trust-tier UI, `GET /api/activity/:userId` unified timeline (#391), `POST /api/sessions/pair/consume` short-lived pairing exchange (#385), `GET /api/connectors/:userId/status` OAuth health (#377), `GET /api/health/live` + `GET /api/health/ready` (#378) including the pool-stats 503 path, `GET /metrics` Prometheus exposition (#392). Each block cites the source file so a future drift is visible at the spec line. Documentation-only — no code change.
