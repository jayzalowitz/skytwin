...
- **Lifebook importance promote/demote ceremony (closes #321, partial).** Migration 056 adds `metadata JSONB NOT NULL DEFAULT '{}'` to `lifebooks`; the existing schema.sql declaration is updated in lock-step so fresh-DB bootstrap matches. The `metadata.importanceOverride` shape is `{ value, setAt, decayDays }` where `decayDays = 0` means "never auto-decay until cleared."
