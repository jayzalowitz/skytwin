...
- **Batched the 10 dependabot bumps opened after #522 into one lockfile regeneration:** `react-native` 0.85.3→0.86.0, `expo-camera` ~56.0.8, `expo-asset` ~56.0.17, `expo-notifications` ~56.0.18, `expo-audio` ~56.0.12, `expo-file-system` ~56.0.8, `@react-navigation/native` ^7.3.3 (mobile); `jsdom` 25→29 (web dev — a 4-major jump, the one to watch); `turbo` 2.9.18, `prettier` 3.8.4 (root dev). Verified: `pnpm build` 35/35, `pnpm lint` 61/61, web tests 64/64 (the jsdom-29 a11y/jsdom-env tests pass), mobile 224 unit tests pass — the only local misses are the `integration-live` suites that need a running API+DB (green in CI). The pre-existing `@expo/dom-webview` peer warning in the expo tree is unchanged. Supersedes dependabot PRs #527–#536.
