...
- **LLM provider chain is now local-first by default (#375 minimal slice).** Pre-fix, the chain ordering in `apps/api/src/lib/llm-client-factory.ts` was `ANTHROPIC → OPENAI → GOOGLE → OLLAMA → EMBEDDED` — the moment a single cloud key was set in the env, the cloud provider became first-call and every decision's prompt (sender, subject, snippet, preferences, patterns, traits, last five episodic memories) left the machine. The privacy policy promised "your data stays local"; the default contradicted that the instant a user configured Anthropic for "smarter decisions." `buildProviderChain` now splits providers into local (embedded + ollama) and cloud (anthropic + openai + google) buckets and emits `local || cloud` by default. New `SKYTWIN_LLM_PRIORITY=cloud-first` env var opts back into the legacy hosted-first ordering — that's the explicit consent gate documented in privacy.html, and the env var itself is the audit trail. Unknown / typo values fall through to local-first (a typo must not silently escalate to cloud). Case-insensitive on the value. New `buildProviderChain` export (previously private) so the ordering invariants are unit-testable. 8 new tests in `apps/api/src/__tests__/llm-client-factory.test.ts` lock the default order, the cloud-first opt-in, the typo fallback, case-insensitivity, empty-env behaviour, and the canonical sub-orders when only one bucket is present. `docs/privacy.html` updated with the new ordering + the opt-in env var as a consent gate. The per-user toggle UI (Settings + first-run wizard) and the prompt-builder PII redaction pass — the other halves of the original #375 spec — are tracked as follow-ups; this PR ships the safer-default piece because the doc is currently lying and a single env-var-aware reorder closes the most acute leak without needing a UI change.
