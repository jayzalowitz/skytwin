...
- **Tiered OAuth scope policy.** Gmail's `readonly` and `modify` are Google-classified **restricted** scopes that require an annual ~$15k–$50k third-party CASA security assessment when accessed through a public OAuth client. To avoid blocking launch on that gate, `resolveRequestedScopes()` in `apps/api/src/routes/oauth.ts` now splits the scope set by config source: the bundled SkyTwin-team client only ever requests `openid`, `email`, `profile`, `calendar.readonly`, and `calendar.events` (sensitive, no third-party fee — clears on standard Google app review in days–weeks). Users who want Gmail features paste their own Google Cloud OAuth credentials into Setup; their own client is private and exempt from app verification, so Gmail scopes light up immediately. Documented end-to-end at `docs/google-verification.md`, with a five-minute user-facing walkthrough at `docs/connect-gmail.html`. Restricted-scope verification for the bundled client is tracked in [#351](https://github.com/jayzalowitz/skytwin/issues/351) — to do when revenue justifies the recurring assessment fee. Six new tests in `apps/api/src/__tests__/oauth-scope-tiers.test.ts` lock in the gating behaviour (bundled drops Gmail with a structured `skipped` report; user-supplied includes it; minimum-scope principle: no Gmail when not asked for, regardless of source). The `/google/authorize` endpoint returns HTTP 412 with code `GMAIL_REQUIRES_BYO_CLIENT` and a `help: '/docs/connect-gmail'` pointer when a caller asks for `?include=gmail` under the bundled client.
