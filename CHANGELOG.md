...
- **No more "draft a reply" to `google-noreply@google.com` (and other compound no-reply senders).** Same bug class as the newsletter-reply fix, one level deeper: a memory-link suggestion offered to draft a reply to an automated Google Terms-of-Service notice. Root cause was a shared anchoring gap in both no-reply detectors — they only recognized the automated token at the **start** of the local part (`noreply@`, `noreply+thread@`, `notifications.42@`), so a hyphen-compound like `google-noreply@` slipped through. The connector then mis-tiered it `inbox_personal` (a human!), and the suggestion engine classified it `received_personal` → `draft_email`. Fixed both: `isAutomatedSender` (`@skytwin/connectors`) now matches a known token as a delimited component anywhere in the local part (regex built from the token set, so the vocabulary stays single-sourced), and the `NO_REPLY_SENDER` fallback (`@skytwin/daily-memory-suggestions`) now also accepts an alphanumeric-hyphen boundary. Both stay conservative: a token embedded without a delimiter (`noreplyfan@`) and a dot-compound that could be a real person (`mary.newsletter@`) are NOT flagged, and any rare false positive errs toward awareness (note, not draft-a-reply) — the safe direction. Correctly tiering these as `inbox_automated` also fixes their #251 retrieval weighting. New tests cover the compound forms, the non-delimited negatives, and the end-to-end `inbox_automated` classification.
