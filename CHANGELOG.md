...
- The authenticated approval endpoint now routes only the exact canonical Inbox archive shape into that boundary. Malformed reserved shapes remain blocked before generic approval side effects, and exact replays return the original recorded response without implying execution.
...
- Added an intentionally unregistered Gmail Inbox mutation service for one owner-bound archive. The service accepts only the versioned canonical archive command, resolves the native Gmail target and account-bound credential behind the repository boundary, and requires an exact in-progress event admission linked to the same owner, decision, candidate, source signal, immutable message reference, verified account, OAuth row, and `gmail.modify` scope. Persisted candidates must match the production serializer's exact six-key shape, remain reversible and verified-zero-cost, and retain untrusted-external provenance. The service is exported for a later approval lifecycle but is not registered with an API, worker, router, or generic action handler. Restore remains a separate durable-compensation slice. See [`gmail-inbox-mutation-port.ts`](./packages/ironclaw-adapter/src/gmail-inbox-mutation-port.ts) and [`gmail-message-ref-repository.ts`](./packages/db/src/repositories/gmail-message-ref-repository.ts).
- A Gmail Inbox archive is bounded to one provider `POST` that removes only `INBOX`. A preflight read avoids unnecessary writes; an ambiguous response permits one read-only reconciliation and never a second mutation. The result carries provider-response time without writing secondary observation state, so the later lifecycle can persist terminal effect truth first. No observation-write API is exposed by this foundation.
...
- Assistant routing now treats explanation persistence as a hard gate before creating an approval or emitting its SSE notification. Every message requires a client request UUID; migration 077 stores it under a user-scoped unique index so sync and SSE HTTP retries reuse the persisted thread, message ID, decision, barrier, approval, and assistant bubble. A safety-pipeline error is surfaced as a deliberate fail-closed non-action instead of being swallowed into ordinary LLM chat.
- Memory auto-execution now records the typed intended `DecisionOutcome`, a pre-effect `ExplanationRecord`, and the exact candidate/risk/policy snapshot before an adapter can run. The final policy gate evaluates the selected adapter's adjusted risk, and dispatch requires a single-use router-issued handle bound to a deep-frozen copy of the plan/risk/action and the exact adapter instance. Fabricated, mutated, reused, or registry-stale handles fail before dispatch. Once the adapter returns a known result, the barrier records it before secondary audit writes; later persistence failures cannot downgrade success/failure to unknown. A terminal explanation is appended after the result without replacing the pre-effect evidence.
- Approved memory-derived actions now use the same prepared-route admission: adapter-adjusted risk is checked by the final policy gate, the typed outcome/explanation and barrier are durable before dispatch, and only the exact single-use handle executes. Ambiguous adapter throws remain `unknown` and suppress replay. Prepared routes reject Map, Set, custom-class, cyclic, and other non-canonical data before an adapter can observe it.
- Memory opportunity claims now atomically move the row to `processing`, so concurrent workers cannot both queue approvals or execute the same opportunity. An interrupted pre-effect evaluation stays `processing` and requires reconciliation rather than being automatically reclaimed.
- Unattended routine registration is disabled for the public beta. An allowed request still persists a typed blocked outcome and explanation, then returns unavailable without contacting IronClaw, because the current scheduler cannot run policy and explanation admission before every later execution. The underlying routine-creation HTTP POST is also single-dispatch; it is never retried without a remote idempotency contract.
- Added a durable, user-scoped `pre_effect_barriers` admission table in migration 080 whose mutations require the owner ID, decision/action/explanation ownership, and a compare-and-set status. Every terminal row carries a persisted explanation UUID. Only one caller can move a prepared effect to `in_progress`; completed duplicate requests return the durable result. A caught ambiguous adapter error becomes terminal `execution_unknown`; a process crash remains `in_progress`. Both states are non-replayable and require reconciliation. This is intentionally **at-most-once**, not exactly-once: the current execution adapter contract does not accept a remote idempotency key, so a process crash during an adapter call cannot prove whether the remote effect happened.
- Event-ingest auto-execution now uses the same prepare, fresh-policy, explanation, barrier, claim, and single-dispatch sequence. Adapter-controlled streaming payloads are not written to the execution ledger or SSE; only bounded terminal status codes cross those boundaries. Concurrent and incomplete duplicate ingests never replay an uncertain effect.
...
- **Two `#281` regression guards were passing vacuously.** `returns no entry for received-only contacts` and `does NOT count cross-day activity` assert only *absence*, which an empty result map satisfies — so they kept passing while the window silently swallowed every one of their fixtures. Both now seed a genuinely bidirectional positive control, so absence has to be meaningful. The whole `computeBidirectionalThreadCounts` suite now runs **twice** via `describe.each` — once on the real clock, once five years ahead under fake timers. Every fixture derives from a `daysAgo()` helper and so travels with the clock; an absolute literal does not, and fails the shifted run. (A first attempt at this guard was itself vacuous — a single extra test with its own inline relative fixtures, which only proved the implementation reads `Date.now()` and stayed green when an absolute date was reintroduced into a sibling test. Caught in review and rebuilt.)

- **Every test suite in `@skytwin/api`, `@skytwin/llm-client`, and the workspace root was running twice.** Vitest 4 removed `**/dist/**` from its built-in `exclude` defaults (v3 shipped it; v4's defaults are only `node_modules` and `.git`). Packages that compile `__tests__/` into `dist/` therefore ran each suite once from source and once from the stale compiled copy — doubling CI wall time and double-reporting every failure while attributing half of them to a build artifact, which is a large part of why the real assertion failure stayed buried. **34 packages** compile `__tests__/` into `dist/`, and a package with no local vitest config inherits the root one — so this affected nearly the whole workspace, not a handful of packages. Restored in the root config and in `apps/api`'s local config, which replaces rather than merges with it (`apps/desktop` already had the exclusion; `apps/web` uses explicit `include` globs; `packages/llm-client`'s local config was byte-identical to the root one and is deleted, so it now inherits future fixes automatically). `@skytwin/api` alone drops from 1,976 executed tests across 160 files to 988 across 80. The trap is now documented in CLAUDE.md, since a package-local config silently disables the exclusion.

- **`evals.yml`: a latent CI bug plus a dead safety gate.** Its `setup-github-actions-caching-for-turbo` step lacked the `continue-on-error: true` that every equivalent step in `build.yml` has, so a GitHub cache-service hiccup could fail the whole eval job. (The turbo sidecar's 400/500 chatter is noisy but harmless — it was *not* the cause of the red `Test` job, despite dominating the logs.)

  Separately, the `Check for safety regressions` step **could never fire**: it re-ran the identical eval suite that the step above it had just run, and without `if: always()` a step is skipped once an earlier one fails — so its `grep` only ever inspected an already-green run. It also piped vitest into `tee` without `set -o pipefail`, and GitHub runs `run: |` blocks under `bash -e` (errexit, *not* pipefail), so the pipeline took tee's exit status of 0. A crashed eval suite — import error, unhandled rejection, "No test files found" — produced no literal `FAIL` for the grep either, and would have reported as a **passing safety gate**. Merged into a single run with `pipefail`, halving eval wall time as a side effect.

- **The public privacy policy no longer claims at-rest encryption we don't actually perform.** `docs/privacy.html` is served from GitHub Pages, linked twice from `docs/index.html`, and registered as the privacy-policy URL on the Google OAuth consent screen — and it told users that SkyTwin envelope-encrypts the `preferences`, `twin_profiles`, and `brain_pages` tables, and that "if your laptop is stolen or your disk is imaged, the contents of your memory and preferences are unreadable without your passphrase." That is false. `setPreferenceVaultKeyProvider` has zero non-test callers, so the DB adapter's `vaultKeyProvider` is permanently `null` and `vault-helper.ts` runs in plaintext mode; the `*_encrypted` columns added by migration `066-encrypt-high-value-tables.sql` are referenced by no shipping code path. The page now states plainly that those three tables (and the derived `content_tsv` / `embedding` fields) are stored **unencrypted**, that the encryption schema exists but is not yet active (tracked as #374), and recommends full-disk encryption (FileVault / BitLocker / LUKS) as the protection that actually applies today. The stolen-laptop sentence and the "encryption activates once you set a vault passphrase" line are gone.
- **OAuth-token encryption is now described as the opt-in, partial feature it is.** The policy previously stated flatly that tokens "are encrypted at rest." In reality `oauthRepository.upsert` writes plaintext `access_token` / `refresh_token` unless the user sets a vault passphrase, and even then `apps/worker/src/index.ts` documents that its `KeyCache` stays empty until cross-process unlock IPC lands (#212 follow-up). Both caveats are now stated on the page.
- **The scopes section no longer advertises restricted Gmail scopes the bundled OAuth client cannot request.** `resolveRequestedScopes` in `apps/api/src/routes/oauth.ts` grants the bundled client identity + Calendar only and refuses to add `gmail.readonly` / `gmail.modify` for it; Gmail requires a bring-your-own OAuth client. Listing restricted scopes a published client never requests invites an avoidable CASA assessment. `docs/privacy.html` and the scope table on `docs/index.html` now split the two tiers explicitly.
- **Microsoft Graph scopes are disclosed.** The shipped Outlook connector requests `User.Read`, `Mail.Read`, `Calendars.Read`, and `offline_access` (bring-your-own Entra app, read-only — no Outlook write scopes), and the policy did not mention Microsoft at all. Added.
- **The default-off desktop crash reporter is disclosed.** `apps/desktop/src/crash-reporter.ts` POSTs a PII-scrubbed report to `https://crash.skytwin.dev/api/desktop/report` when the user opts in, while the policy named `skytwin.dev` as a destination nothing is sent to. The page now describes the setting, the exact payload fields, the `redactPii` scrubbing, and that it is off unless you turn it on.
- **Internal audit docs are excluded from the GitHub Pages build.** `docs/launch-readiness-report.md` and `docs/launch-plan.md` were being published on the same origin as the privacy policy, in candid engineering language that contradicted it. Both are now in the `exclude` list in `docs/_config.yml`; they remain readable on GitHub.
- **Effective / last-updated dates bumped to 2026-08-27.**
...
- **`evals.yml`: a latent CI bug plus a dead safety gate.** Its `setup-github-actions-caching-for-turbo` step lacked the `continue-on-error: true` that every equivalent step in `build.yml` has, so a GitHub cache-service hiccup could fail the whole eval job. (The turbo sidecar's 400/500 chatter is noisy but harmless — it was *not* the cause of the red `Test` job, despite dominating the logs.)
  Separately, the `Check for safety regressions` step **could never fire**: it re-ran the identical eval suite that the step above it had just run, and without `if: always()` a step is skipped once an earlier one fails — so its `grep` only ever inspected an already-green run. It also piped vitest into `tee` without `set -o pipefail`, and GitHub runs `run: |` blocks under `bash -e` (errexit, *not* pipefail), so the pipeline took tee's exit status of 0. A crashed eval suite — import error, unhandled rejection, "No test files found" — produced no literal `FAIL` for the grep either, and would have reported as a **passing safety gate**. Merged into a single run with `pipefail`, halving eval wall time as a side effect.
- **Desktop auto-update can actually fire — the packaged app version is no longer frozen at `0.3.0`.** electron-builder stamps both artifact filenames and the `latest*.yml` update manifests from `apps/desktop/package.json#version`, which has been pinned to `0.3.0` since #31 (it can't mirror the repo's four-segment `VERSION` — electron-builder rejects those). So every release published `0.3.0` and electron-updater's semver compare against an installed `0.3.0` answered "no update available" forever; the published assets show it (`SkyTwin-0.3.0-arm64.dmg`, `SkyTwin.Setup.0.3.0.exe`). CI now derives a three-segment version from `VERSION` — `major.minor.(patch * 100 + build)`, so `0.6.101.0` → `0.6.10100` — in a new `.github/scripts/derive-app-version.sh`, and each desktop job injects it with `--config.extraMetadata.version`. The encoding is injective and strictly monotonic over the repo's whole `VERSION` history (both properties are asserted in tests against that real history); the script hard-fails rather than emitting a colliding version if the build segment ever reaches 100, or if `VERSION` isn't four numeric segments. The checked-in `0.3.0` is left alone so local packaging still works with no setup. See `docs/release-procedure.md`.
- **Upgrading via a newer `.dmg`/`.exe` no longer silently runs the previous release's backend.** `ServiceManager.ensureEmbeddedRoot()` used `app.getVersion()` as the cache marker for the extracted `<userData>/embedded/` API/worker/web tree — and that version was the frozen `0.3.0`, so the marker matched across upgrades and the new Electron shell kept running the stale extracted backend. The marker is now the bundle's own sha256 (`bundleId`, written into `bundle-manifest.json` by `build-single-binary.sh`), with `bundledAt` → tarball size+mtime → app version as ordered fallbacks (`apps/desktop/src/bundle-marker.ts`). Content-addressed, so it invalidates exactly when the bundle changes and never on a version string that didn't.
...
- **Safety invariant: your autonomy settings now actually reach the policy engine on the main ingest path.** `DecisionMaker.evaluate` called `PolicyEvaluator.evaluate` with four arguments and dropped the fifth (`autonomySettings`), so every signal that came through `POST /api/events/ingest` — the primary pipeline — was evaluated as if the user had no settings at all. Everything the evaluator gates on that argument was skipped: the **pause kill switch** (#379: "Auto-execution paused by user" was decorative on this path), the **domain allow/block lists**, the **per-action spend cap**, the `costZeroIntent === 'unknown'` escalation (#372), `requireApprovalForIrreversible`, and **quiet hours**. `DecisionContext` now carries `autonomySettings`, `apps/api/src/routes/events.ts` populates it from the user row, and the decision maker forwards it.
  **Blast radius, honestly stated:** the built-in `NO_SPEND_WITHOUT_LIMIT` policy and the observer/suggest hard-return in `shouldAutoExecute` still held, so nothing that could spend money and nothing at the two lowest trust tiers escaped. What did escape the user's own settings was reversible, zero-cost, low-risk actions (`label_email`, `archive_email`, `accept_invite`) for users who had manually raised their own tier to `low_autonomy` or above.
  The kill-switch ordering inside the evaluator is unchanged and still correct: the pause is captured up front but **applied last**, so it only escalates actions that would otherwise have been allowed — it never turns a deny into an approval and never strips the injection-guard `confirmationLevel` (the regression Copilot caught on PR #421). There is now a test pinning that.
- **`readAutonomy` no longer silently drops the pause and quiet-hours fields.** `apps/api/src/cost-gate.ts` rebuilt the settings as an object literal listing five fields, discarding `paused`, `pausedAt`, `pausedReason`, `quietHoursStart`, and `quietHoursEnd`. Callers that *did* pass the settings to the evaluator correctly — `/api/routines` — were therefore still pause-inert and quiet-hours-inert. Both hand-rolled readers (the API one and a divergent copy in `apps/worker/src/jobs/memory-action-loop.ts`) now delegate to one shared, tested parser, `parseAutonomySettings` in `@skytwin/shared-types`, which carries every field and falls back per-field (never coercing a string spend cap into a number, never reading a truthy non-boolean as `paused`).
- **The post-approval policy check in `apps/api/src/routes/approvals.ts` now matches its own comment.** It claimed "spend limits, domain restrictions still apply" while calling `evaluate()` with three arguments — dropping both the risk assessment and the autonomy settings, so the per-action spend cap and the domain lists never ran and risk-keyed policy rules matched nothing. Both are now passed.

- **Two fail-open paths found by codex review, both fixed before merge.**
  - *Quiet hours could downgrade a policy deny into an approval prompt.* `PolicyEvaluator` early-returned from its quiet-hours branch **before** the policy-evaluation loop, so with autonomy settings now reaching the ingest path, an active quiet-hours window turned a policy `deny` into `allowed: true, requiresApproval: true`. Quiet hours is now recorded and applied at the bottom alongside the kill switch, for the same documented reason the kill switch is deferred: escalating to approval is *weaker* than denying, so it must not short-circuit ahead of a deny path.
  - *A malformed allowlist became "allow everything".* `checkDomainAllowlist` reads a non-empty `allowedDomains` as "the domain must be in this list" and an empty one as "no restriction". The de-duplicated parser filtered non-strings out, so `allowedDomains: [42]` — which previously denied every domain, since no string equals `42` — narrowed to `[]` and silently became unrestricted. It now fails closed: a list that had entries but loses all of them to narrowing keeps one unmatchable entry.
...
- **No-code routines — the scheduling + matching engine (#519, part 3a).** Two pure functions in `@skytwin/routines`, the foundation the worker scheduler (part 3b) will consume:
  - `computeNextRun(spec, from, tz?)` — a Watch's next firing time. `hourly` is +1h; `daily`/`weekly` fire at `hourOfDay` (default 8) — and, for weekly, on `dayOfWeek` — computed in the user's **IANA timezone** (so "every morning at 8am" means 8am *where the user is*, not 8am UTC). Timezone math uses only native `Intl` (no date library), with a **two-pass DST correction** so 8am stays 8am even on spring-forward / fall-back days; an invalid IANA zone falls back to UTC, an invalid `from` date fails loud, and an out-of-range `dayOfWeek` is normalized — the pure function stays total.
  - `matchesFilter(signal, filter)` — does a signal match a Watch's `RoutineFilter`? AND across `sources`/`fromContains`/`keywords`/`domains`, OR within each; case-insensitive. `domains` are topical tags **expanded to related terms** (a `security` watch catches phishing / suspicious / fraud, not only the literal word). An empty filter matches everything — but the API forbids an *active* empty-filter watch, so the scheduler only runs this on narrowed filters.

  Deterministic and fully unit-tested (24 cases: hourly/daily/weekly rollover, timezone correctness incl. a non-UTC zone + DST spring-forward/fall-back, invalid-tz/invalid-date/out-of-range-day totality, steady-state 24h spacing; source/sender/keyword/domain matching + expansion, AND-across / OR-within). Part 3b wires these into a gated worker scheduler that fires due watches → matches signals → writes a `watch_run` + explanation.
