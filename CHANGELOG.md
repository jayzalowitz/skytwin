...
- **Release evidence now fails closed on incomplete or advisory-only material.** The verifier requires an SPDX 2.3 release profile with document namespace, recognized relationships, analyzed packages, and package-to-file coverage; exact subject digests; byte-exact runnable checksum and provenance instructions; cryptographic GitHub attestation verification bound to the tag source; and a protected release environment with self-review and administrator bypass disabled. A pinned native platform matrix must execute each reviewed machine-verifier command and an exclusive dependent job must aggregate its immutable reports; signing proof is split across macOS, Windows, and Linux. Publication also rejects a tagged commit that is not merged into `main`, and the release checkout does not persist its write-capable token in Git configuration. Verifier implementations, CI-result generation, signing, and provenance remain blocked prerequisites; this gate does not claim those materials exist yet.
