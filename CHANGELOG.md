...
- **LLM-generated candidates no longer silently bypass the spend cap (#372).** Pre-fix, `packages/llm-client/src/response-parser.ts:112` hardcoded `estimatedCostCents = 0` on every LLM-generated `CandidateAction` and `packages/policy-engine/src/policy-evaluator.ts:199` short-circuited any action with `<= 0` to "allowed." Combined: a user setting `dailyCapCents = 1000` ($10/day) saw the twin run LLM-suggested actions with combined real cost well past $10 because zero was treated as "free, auto-approve." `CandidateAction` gains a `costZeroIntent?: 'verified_zero' | 'unknown'` discriminator (`packages/shared-types/src/decision.ts`). The LLM parser now emits `costZeroIntent: 'unknown'` to mark "the LLM does not get to declare its own price." `PolicyEvaluator.checkSpendLimit` returns `false` whenever intent is `'unknown'`, forcing the candidate to escalate to the human rather than auto-approving on a default zero. Undefined intent (legacy rule-based generators in `decision-maker.ts`, `sender-aware-candidates.ts`, `draft-email-candidate.ts`, all of which emit genuinely-free actions) is treated as `'verified_zero'` so backward compatibility is preserved. The pre-fix `response-parser.test.ts:163` lock-in test was updated to assert `costZeroIntent === 'unknown'`; three new tests in `policy-evaluator.test.ts` cover the unknown→reject, verified_zero→fast-path, and undefined→fast-path matrix. Safety Invariant #4.
